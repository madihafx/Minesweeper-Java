...
public class Minesweeper {
   ...
   // In-game symbols
    static final String HIDDEN = " ? ";  // Solid block
    static final String BOMB   = " X ";  // Explosion
    static final String EMPTY  = "   ";  // Clean space
    static final String FLAG   = " F ";  // Flag
             ...
             // execute FLAG action
               if (action.equals("F")) {
                   if (!flagged[r][c] && countFlags(flagged, size) >= numberOfBombs) {
                       ...
                       System.out.println("  Warning: you have already placed " + numberOfBombs + " flag(s) - equal to the total number of bombs!");
...
System.out.println("  - An empty cell [ ] means zero bombs are nearby.");


System.out.println("    Revealing a [ ] auto-reveals its neighbors too!");


       System.out.println("  - Use flags “F” to mark cells you think are bombs.");
...
System.out.println("  [ ]  => revealed, 0 bombs nearby");


       System.out.println("  [1]   => revealed, 1 bomb nearby  (up to [8])");
       System.out.println("  [X]  => BOMB (shown when game ends)");
       System.out.println("  [F]  => flagged by you");
       System.out.println();
...
System.out.println("  3  [?]  [F]   [?]");
   ...
   static int chooseBombCount(Scanner sc, int size) {
       ...
       while (true) {
           try {
               ...
               if (choice == 1) { System.out.println("  Easy selected!\n");   return easyBombs;   }
               if (choice == 2) { System.out.println("  Medium selected!\n"); return mediumBombs; }
               if (choice == 3) { System.out.println("  Hard selected!\n");   return hardBombs;   }
   ...
   static void reveal(int[][] matrix, boolean[][] revealed, int r, int c, int valueOfBomb, int size) {
       if (revealed[r][c]) return; // base case: already revealed, stop here
